...
class RNNLM_Model(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      with tf.variable_scope('embed', reuse=tf.AUTO_REUSE):
        ...
        # inputs_embeddings = tf.nn.embedding_lookup(embeddings, self.input_placeholder)

        inputs = []
        for i in range(self.config.num_steps):
          inputs = inputs + [tf.nn.embedding_lookup(embeddings, self.input_placeholder[:,i])]
  ...
  def add_projection(self, rnn_outputs):
    ...
    with tf.variable_scope('projection', reuse=tf.AUTO_REUSE):
      ...
      b_2 = tf.get_variable('b_2', [1, len(self.vocab)])
    outputs = []
    for i in range(self.config.num_steps):
      outputs_ = tf.add(tf.matmul(rnn_outputs[i], U), b_2)
  ...
  def add_loss_op(self, output):
    ...
    """
    ### YOUR CODE HERE
    onehot_labels = tf.one_hot(indices=tf.cast(self.labels_placeholder, tf.int32), depth=len(self.vocab))
    loss = tf.nn.softmax_cross_entropy_with_logits(labels=onehot_labels, logits=output)
    ### END YOUR CODE
    return loss
  ...
  def add_model(self, inputs):
    ...
    with tf.variable_scope('RNN', reuse=tf.AUTO_REUSE):
      ...
      b_1 = tf.get_variable('b_1', [1, self.config.hidden_size])
    rnn_outputs = []
    h_ = self.initial_state
    for i in range(self.config.num_steps):
      inputs_ = inputs[i]
      tmp_ = tf.add(tf.matmul(h_, H) + tf.matmul(inputs_, I), b_1)
      h_ = tf.sigmoid(tmp_)
      rnn_outputs = rnn_outputs + [h_]
    self.final_state = h_
    ### END YOUR CODE
    return rnn_outputs
