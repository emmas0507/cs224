...
class NERModel(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      with tf.variable_scope('embed', reuse=tf.AUTO_REUSE):
        embeddings = tf.get_variable('embeddings', [len(self.wv), self.config.embed_size])
  ...
  def add_model(self, window):
    ...
    with tf.variable_scope('model', reuse=tf.AUTO_REUSE):
        W = tf.get_variable('W', [self.config.window_size*self.config.embed_size, self.config.hidden_size], regularizer=regularizer)
        b1 = tf.get_variable('b1', [1, self.config.hidden_size])
        affine = tf.add(tf.matmul(window, W), b1)
        h = tf.tanh(affine)
        h_dropout = tf.nn.dropout(h, keep_prob=self.config.dropout)
        U = tf.get_variable('U', [self.config.hidden_size, self.config.label_size], regularizer=regularizer)
        b2 = tf.get_variable('b2', [1, self.config.label_size])
        output = tf.add(tf.matmul(h_dropout, U), b2)
        # output = tf.nn.softmax(affine2)
  ...
  def add_loss_op(self, y):
    ...
    ### YOUR CODE HERE
    # raise NotImplementedError
    loss = tf.losses.softmax_cross_entropy(self.labels_placeholder, y)
    # reg_variables = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)
    # reg_term = tf.contrib.layers.apply_regularization(self.config.l2, reg_variables)
    # print reg_term
  ...
  def run_epoch(self, session, input_data, input_labels,
    ...
    for step, (x, y) in enumerate(
      ...
      # print 'step:{}, x example:{}, y example: {}'.format(step, x[0:3], y[0:3])
