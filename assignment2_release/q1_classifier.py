...
class SoftmaxModel(Model):
  ...
  def add_placeholders(self):
    ...
    self.labels_placeholder = tf.placeholder(tf.float32, [None, None])
    ### END YOUR CODE
  ...
  def add_model(self, input_data):
    ...
    ### YOUR CODE HERE
    # raise NotImplementedError
    # [batch_size, n_features] = tf.shape(input_data)
    with tf.variable_scope('model', reuse=tf.AUTO_REUSE):
        w = tf.get_variable('w', [self.config.n_features, self.config.n_classes])
        b = tf.get_variable('b', [1, self.config.n_classes])
        affine = tf.add(tf.matmul(input_data, w), b)
        out = tf.nn.softmax(affine)
    ### END YOUR CODE
    return out
  ...
  def add_loss_op(self, pred):
    ...
    ### YOUR CODE HERE
    # raise NotImplementedError
    loss = tf.reduce_sum(-tf.reduce_sum(tf.multiply(tf.log(pred), self.labels_placeholder), reduction_indices=[1]))
    ### END YOUR CODE
    return loss
