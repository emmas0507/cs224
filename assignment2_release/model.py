...
class Model(object):
  ...
  def add_loss_op(self, pred, label):
    ...
    """
    # loss = tf.reduce_sum(-tf.reduce_sum(tf.multiply(tf.log(pred), label), reduction_indices=[1]))
    raise NotImplementedError("Each Model must re-implement this method.")
    # return loss
  ...
  def run_epoch(self, sess, input_data, input_labels):
    ...
    """
    # loss = self.fit(sess, input_data, input_labels)
    # optimizer = tf.train.GradientDescentOptimizer(learning_rate=0.1).minimize(loss)
    # feed_dict_ = self.create_feed_dict(input_data, input_labels)
    # [loss_val, _] = sess.run([loss, optimizer], feed_dict=feed_dict_)
  ...
  def fit(self, sess, input_data, input_labels):
    ...
    """
    raise NotImplementedError("Each Model must re-implement this method.")
    # input, labels = self.add_placeholders()
    # pred = self.add_model(input)
    # loss = self.add_loss_op(pred, labels)
    # return loss
