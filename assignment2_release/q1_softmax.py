...
def softmax(x):
  ...
  ### YOUR CODE HERE
  x_norm = tf.subtract(x, tf.reduce_max(x, axis=1, keep_dims=True))
  x_exp = tf.exp(x_norm)
  x_row_sum = tf.reduce_sum(x_exp, axis=1, keep_dims=True)
...
def cross_entropy_loss(y, yhat):
  ...
  ### YOUR CODE HERE
  y_ = tf.to_float(y)
  out = - tf.reduce_sum(tf.log(tf.reduce_sum(tf.multiply(y_, yhat), axis=1)))
  ### END YOUR CODE
  return out
