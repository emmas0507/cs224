...
def getSentenceFeature(tokens, wordVectors, sentence):
    ...
    ### YOUR CODE HERE
    sentence_index = [tokens[w_] for w_ in sentence]
    sentVector = wordVectors[sentence_index,:].mean(axis=0)
    ### END YOUR CODE
...
def softmaxRegression(features, labels, weights, regularization = 0.0, nopredictions = False):
    ...
    cross_entropy_grad = prob
    cross_entropy_grad[range(N), labels] = cross_entropy_grad[range(N), labels] - 1
    grad = np.dot(np.transpose(features), cross_entropy_grad) + regularization * weights
